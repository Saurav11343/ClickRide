...
export default function ManageVehicle() {
  ...
  useEffect(() => {
    ...
    const foundRequest = totalUpdateResponce.find(
      (response) => response._id === requestId,
    );
    if (foundRequest) {
    ...
    const pendingRequest = totalUpdateResponce.find(
      (request) =>
      ...
      if (UpdateRequest && UpdateRequest.success) {
        ...
        toast.error(
          UpdateRequest?.message ||
            "Failed to send update vehicle data request.",
        );
      }
              ...
              <div className="grid lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-1 gap-6">
                ...
                <div className="space-y-1.5">
                  <div>
                    ...
                    />
                    <p>
                      Status: <strong>{watch("availabilityStatus")}</strong>
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="relative bg-white p-6 rounded-lg w-11/12 sm:w-96 md:w-3/4 lg:w-1/2 xl:w-1/3 max-h-[90vh] overflow-auto scrollbar-hide mx-4 sm:mx-8">
                      ...
                      <MapSelector
                        ...
                        initialCoordinates={{ lat: latitude, lng: longitude }}
